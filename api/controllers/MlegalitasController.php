...
use Yii;
use app\models\TblMonitoringFlegalitas;
...
class MlegalitasController extends Controller {
    ...
    public function actionKode() {
        ...
        $query = new Query;
        $query->from('tbl_monitoring_flegalitas')
                ->select('*')
                ->orderBy('no_mflegalitas DESC')
                ->limit(1);

        $command = $query->createCommand();
        $models = $command->queryOne();
        $urut = (empty($models)) ? 1 : ((int) substr($models['no_mflegalitas'], -5)) + 1;
        $kode = 'MFL' . substr('00000' . $urut, -5);
...
//                }
    ...
    public function actionCreate() {
        $params = json_decode(file_get_contents("php://input"), true);
        $model = new TblMonitoringFlegalitas();
    ...
    protected function findModel($id) {
        if (($model = TblMonitoringFlegalitas::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionCari() {
        ...
        $query->from('tbl_monitoring_flegalitas')
                ->select("*")
                ->where(['like', 'no_mflegalitas', $params['nama']]);
