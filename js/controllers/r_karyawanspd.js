app.controller('karyawanSpdCtrl', function ($scope, Data, toaster) {
    ...
    $scope.print = function (form) {
        if ('tanggal' in form && form.tanggal.startDate != null) {
            Data.post('karyawanspd/rekap', form).then(function (data) {
                window.open('api/web/karyawanspd/excel?print=true', "", "width=500");
            });
        } else {
...
//        if ($query.length >= 3) {
//            Data.get('karyawanspd/cari', {karyawanspd: $query}).then(function (data) {
//                $scope.resultskaryawanspd = data.data;
//            });
//        }
//    };
