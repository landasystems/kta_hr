app.controller('ijazahKeluarCtrl', function ($scope, Data, toaster) {
    //init data
    var tableStateRef;
    var paramRef;
    $scope.form = {};
    ...
    $scope.print = function (form) {
        if ('tanggal' in form && form.tanggal.startDate != null) {
            Data.post('ijazah/rekapkeluar', form).then(function (data) {
                window.open('api/web/ijazah/excelkeluar?print=true', "", "width=500");
            });
        } else {
    ...
    $scope.excel = function (form) {
        if ('tanggal' in form && form.tanggal.startDate != null) {
            Data.post('ijazah/rekapkeluar', form).then(function (data) {
                window.location = 'api/web/ijazah/excelkeluar';
            });
        } else {
            toaster.pop('error', "Terjadi Kesalahan", "Masukkan periode terlebih dahulu");
        }
