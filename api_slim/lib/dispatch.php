...
function pagination($item_count, $limit, $cur_page, $link) {
    if ($limit > 0 && $item_count > 0) {
        ...
        $paginate = '<div class="blog-section"><ul class="page_pagination center sec-padd-bottom">'; //pagination pagination-blog
        if ($page_count != 1) {
            //if ($cur_page == 1) { $paginate .= '<li><a>Awal</a></li>'; }
            if ($cur_page > 1) {
                $url = $link . addOrUpdateUrlParam('page', 1);
                $paginate .= '<li><a href="' . urldecode($url) . '">Awal</a></li>';
            }

            //if ($cur_page > 1) { $url = $link . addOrUpdateUrlParam('page', $cur_page - 1); $paginate .= '<li><a href="' . urldecode($url) . '">Sebelum</a></li>'; }
            ...
            for ($i = $start; $i <= $end; $i++) {
                if ($i == $cur_page)
                    $active = 'class="trans3s" style="background-color: rgba(32, 160, 32, 1)"';
                else
                    $active = 'class="trans3s"';
            ...
            //if ($cur_page != $page_count) { $paginate .= '<li><a href="' . urldecode($url) . '">Berikut</a></li>'; }

            //if ($cur_page == $page_count) { $paginate .= '<li><a>Akhir</a></li>'; }
            if ($cur_page < $page_count) {
                $url = $link . addOrUpdateUrlParam('page', $page_count);
                $paginate .= '<li><a href="' . urldecode($url) . '">Akhir</a></li>';
            }
